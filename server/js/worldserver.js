...
class World {
    ...
    spawnStaticEntities() {
        ...
        Object.keys(this.map.staticEntities || {}).forEach(function (tid) {
            ...
            var kind = Types.getKindFromString(kindName), pos = self.map.tileIndexToGridPosition(Number.parseInt(tid, 10));
            if (Types.isNpc(kind)) {
    ...
    getDroppedItem(mob) {
        var kind = Types.getKindAsString(mob.kind), drops = Properties[kind].drops, v = Utils.random(100), p = 0, item = null;
