...
### 2026-06-11 15:35 UTC - Manual Tiled Authoring Slice 126

- Status: done
- Scope:
  - Enrich the `tree_1` overlay at tile bounds `(8, 238)` through `(11, 243)` in `render_world/village_biome/houses`.
  - Enrich the partial/right-edge `well_1` assembly at tile bounds `(81, 241)` through `(83, 245)` in `render_world/village_biome/houses`.
  - Reuse existing source `PropTile`/`StructureTile` metadata for all selected tree and well GIDs; do not edit `tilesheet.wang.tsj`.
  - Preserve the nearby tagged blue/red house objects, one-way portal, rat roaming area, checkpoint, terrain, and cliff objects.
  - Preserve object positions, GIDs, layer membership, gameplay markers, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the southwest tree overlay and east well assembly.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_tree_overlay_slice126_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_tree_overlay_slice126_nomarkers.png`.
  - Generated `.data/map-authoring-crops/world_village_well_overlay_slice126_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_well_overlay_slice126_nomarkers.png`.
  - Visual inspection confirmed the left cluster is a tree overlay on the southwest house edge and the right cluster is a separate small well/shadow assembly beside the red house/cliff edge.
  - Source inspection confirmed selected `tree_1` and `well_1` source tiles already carry semantic metadata.
  - Added object-level `PropMetadata` to 22 selected `tree_1_8_238` objects and 15 selected `well_1_81_241` objects.
  - Re-ran the audit after an initial pass exposed five omitted cells, then folded those cells into the same two instances.
- Evidence:
  - Source check confirmed `tree_1_8_238` has 22 objects and `well_1_81_241` has 15 objects with `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, expected `prop_family`, expected `prop_kind`, and matching `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for the selected `tree_1_8_238` and `well_1_81_241` object IDs.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the two-tile village object around `(31, 236)` using GIDs 785 and 805, then continue to the deadlands rock clusters.

### 2026-06-11 15:25 UTC - Manual Tiled Authoring Slice 125

- Status: done
- Scope:
  - Enrich the large `house_blue` exterior at tile bounds `(59, 213)` through `(66, 222)` in `render_world/village_biome/houses`.
  - Add missing source `StructureTile` metadata for the four left-edge `house_blue` source tiles used by this footprint: GIDs 1492, 1592, 1612, and 1632.
  - Add object-level `PropMetadata` to the 78 matching placed `house_blue` objects with `prop_instance_id=house_blue_59_213`.
  - Exclude the nearby village agent, spawn checkpoint, rat roaming area, tree overlay, path/terrain, and unrelated deadlands objects.
  - Preserve object positions, GIDs, layer membership, gameplay markers, and runtime behavior.
- Ticket:
  - done: Add source and placed-object metadata for the large east-village blue house exterior.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_house_blue_large_slice125_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_house_blue_large_slice125_nomarkers.png`.
  - Visual inspection confirmed a single complete large blue house exterior.
  - Object inspection found no aligned house-entry door; nearby agent/checkpoint/roaming-area objects are separate gameplay markers.
  - Source inspection found four missing `StructureTile` records on the selected left edge; all other selected source tiles are already typed as `house_blue`.
  - Added source `StructureTile` metadata for GIDs 1492, 1592, 1612, and 1632 as non-occluding left-edge `house_blue` padding pieces.
  - Added object-level `PropMetadata` to 78 matching placed `house_blue` objects with `prop_instance_id=house_blue_59_213`.
- Evidence:
  - Source check confirmed GIDs 1492, 1592, 1612, and 1632 now use `class:"StructureTile"`, `type:"house_blue"`, `tile_kind=structure`, `occlusion_kind=none`, and `render_height=0`.
  - Source check confirmed 78 objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_blue`, `prop_kind=house_exterior`, and `prop_instance_id=house_blue_59_213`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the next village tree/well overlay fragments around `(8..11, 238..245)` and `(81..83, 241..245)`, then continue to the deadlands rock clusters.

### 2026-06-11 15:20 UTC - Manual Tiled Authoring Slice 124

- Status: done
- Scope:
  - Enrich four low `beach_fence` structure runs currently placed on `render_world/village_biome/houses` near the beach boundary.
  - Target object IDs 2522 through 2525, 2526 through 2531, 2532 through 2537, and 2538 through 2543.
  - Reuse existing `beach_fence` source `StructureTile` metadata for GIDs 736 through 740; do not edit `tilesheet.wang.tsj`.
  - Add stable per-run `prop_instance_id` values while preserving the nearby checkpoint, guard, sword-cache, palm/root, terrain, and house objects as separate objects.
  - Preserve object positions, GIDs, layer membership, gameplay markers, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the beach-boundary fence runs.
- Key actions:
  - Generated `.data/map-authoring-crops/world_beach_fences_slice124_objects.png`.
  - Generated `.data/map-authoring-crops/world_beach_fences_slice124_nomarkers.png`.
  - Visual inspection confirmed four separate low fence runs along the beach/cliff edge.
  - Source inspection confirmed GIDs 736 through 740 already carry `beach_fence` `StructureTile` metadata.
  - Existing metadata convention from `beach_fence_66_281` uses `prop_family=beach_fence`, `prop_kind=fence`, `biome=beach`, and `depth_mode=collision`.
  - Added object-level `PropMetadata` to 22 fence objects across `beach_fence_59_253`, `beach_fence_16_255`, `beach_fence_43_255`, and `beach_fence_51_255`.
- Evidence:
  - Source check confirmed the four selected fence runs have counts 4, 6, 6, and 6 with `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=beach_fence`, `prop_kind=fence`, and expected `prop_instance_id` values.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the larger `house_blue` structure around `(59..66, 213..220)`, then continue to the later deadlands rock clusters.

### 2026-06-11 15:15 UTC - Manual Tiled Authoring Slice 123

- Status: done
- Scope:
  - Enrich two complete house exteriors in `render_world/village_biome/houses`.
  - Instance A is the southwest blue-roof `house_blue_2` at tile bounds `(3, 241)` through `(10, 248)` with `prop_instance_id=house_blue_2_3_241`.
  - Instance B is the east/beach-edge red-roof `house_red_1` at tile bounds `(75, 241)` through `(82, 250)` with `prop_instance_id=house_red_1_75_241`.
  - Reuse existing source `StructureTile` metadata for both house families; do not edit `tilesheet.wang.tsj`.
  - Exclude nearby tree, well overlay, rat roaming area, one-way portal, sword cache marker, cliff/terrain, and beach gameplay markers from this house metadata slice.
  - Preserve object positions, GIDs, layer membership, gameplay markers, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the next southwest blue and east red village house exteriors.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_southwest_southeast_houses_slice123_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_southwest_southeast_houses_slice123_nomarkers.png`.
  - Visual inspection confirmed two separate complete house exteriors rather than one connected structure.
  - Object inspection found no aligned house-entry door for either instance; `world_portal_77_237` is a separate one-way portal and remains outside the house exterior metadata.
  - Source inspection confirmed all selected `house_blue_2` and `house_red_1` source tiles already carry `StructureTile` metadata.
  - Added object-level `PropMetadata` to 64 matching `house_blue_2_3_241` objects and 80 matching `house_red_1_75_241` objects.
- Evidence:
  - Source check confirmed `house_blue_2_3_241` has 64 objects and `house_red_1_75_241` has 80 objects with `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, expected `prop_family`, expected `prop_kind=house_exterior`, and matching `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for the selected `house_blue_2_3_241` and `house_red_1_75_241` object IDs.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  - `bun run check:world-map:target`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the next village `houses` blank-object cluster around GIDs 736 through 740 at `(59..62, 253)` and repeated groups around y255, then continue to the later deadlands rock clusters.

### 2026-06-11 15:07 UTC - Manual Tiled Authoring Slice 122

- Status: done
- Scope:
  - Enrich the linked red-roof `house_red_1` exterior for `house_40` in `render_world/village_biome/houses`.
  - Target the visually confirmed open-door footprint around tile bounds `(35, 237)` through `(42, 246)` and use `prop_instance_id=house_40_exterior`.
  - Preserve the existing `gameplay_markup/doors` object `world_house_40_entry`, including its `target_map=house_40` and `target_door=house_40_entry` linkage.
  - Reuse existing source `StructureTile` metadata for `house_red_1`, including open-door GIDs 1661 through 1663 and 1681 through 1683; do not edit `tilesheet.wang.tsj` unless validation exposes a source metadata gap.
  - Preserve object positions, GIDs, layer membership, surrounding paths/trees/terrain, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the linked `house_40` red-roof exterior.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_house40_slice122_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_house40_slice122_nomarkers.png`.
  - Visual inspection confirmed a complete red-roof exterior with an open centered doorway.
  - Gameplay inspection found door `world_house_40_entry` at tile `(38, 245)`, targeting map `house_40` and door `house_40_entry`.
  - Spawned a read-only Spark explorer to inspect the next red/blue house candidates after this slice while the local Slice 122 edit proceeds.
  - Added object-level `PropMetadata` to 71 matching placed house objects with `prop_instance_id=house_40_exterior`.
  - Used `door_open` tagging and `open_*` asset parts for the six open-door override tiles while preserving the linked gameplay door object.
- Evidence:
  - Source check confirmed 71 objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_red_1`, `prop_kind=house_exterior`, and `prop_instance_id=house_40_exterior`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the next red/blue house clusters around `(75..82, 241..250)` and `(5..12, 242..249)`.

### 2026-06-11 15:01 UTC - Manual Tiled Authoring Slice 121

- Status: done
- Scope:
  - Enrich the complete 2x2 gray rock prop at tile bounds `(18, 235)` through `(19, 236)` in `render_world/village_biome/houses`.
  - Reuse existing `small_rock_2` source `PropTile` metadata for GIDs 257, 258, 277, and 278; do not edit `tilesheet.wang.tsj`.
  - Add object-level `PropMetadata` to object IDs 2349, 2350, 2365, and 2366 with `prop_instance_id=small_rock_2_18_235`.
  - Preserve object positions, GIDs, source collision objectgroups, nearby tree/gameplay markers, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the repeated village `small_rock_2` boulder.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice121_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice121_nomarkers.png`.
  - Visual inspection confirmed a standalone gray 2x2 boulder near the southwest village path.
  - Source inspection confirmed GIDs 257, 258, 277, and 278 already carry `small_rock_2` `PropTile` metadata and collision objectgroups.
- Evidence:
  - Source check confirmed object IDs 2349, 2350, 2365, and 2366 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=small_rock_2`, `prop_kind=rock`, and `prop_instance_id=small_rock_2_18_235`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the next house/structure cluster around `(35..40, 241..248)`, then continue with red/blue house clusters around `(75..82, 241..250)` and `(5..12, 242..249)`.

### 2026-06-11 14:56 UTC - Manual Tiled Authoring Slice 120

- Status: done
- Scope:
  - Enrich two complete `house_blue_2` exteriors in `render_world/village_biome/houses`.
  - Instance A spans tile bounds `(47, 230)` through `(54, 237)` with `prop_instance_id=house_blue_2_47_230`.
  - Instance B spans tile bounds `(60, 231)` through `(67, 238)` with `prop_instance_id=house_blue_2_60_231`.
  - Reuse existing source `StructureTile` metadata for `house_blue_2`; do not edit `tilesheet.wang.tsj`.
  - Exclude nearby tree, camp, and roaming-area objects from this house slice.
  - Preserve object positions, GIDs, layer membership, nearby overlays/gameplay areas, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for two southeast/east village `house_blue_2` exteriors.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_blue_houses_slice120_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_blue_houses_slice120_nomarkers.png`.
  - Visual inspection confirmed two complete blue-roof house exteriors with nearby trees and rat roaming areas.
  - Gameplay inspection found nearby rat roaming areas but no aligned door objects.
  - Source inspection confirmed the `house_blue_2` 8x8 source grid already has complete `StructureTile` metadata.
  - A Spark explorer side task was started for independent confirmation but did not return before local validation completed; it was closed without using its result.
- Evidence:
  - Source check confirmed each instance has 64 objects with `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_blue_2`, `prop_kind=house_exterior`, and the expected `prop_instance_id`.
  - Terrain authoring audit reports 0 findings for the selected `house_blue_2_47_230` and `house_blue_2_60_231` object IDs.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  - `bun run check:world-map:target`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the repeated `small_rock_2` instance around `(18..19, 235..236)`, then continue with the next house clusters around `(35..40, 241..248)`, `(75..82, 241..250)`, and `(5..12, 242..249)`.

### 2026-06-11 14:51 UTC - Manual Tiled Authoring Slice 119

- Status: done
- Scope:
  - Enrich the complete 2x2 white ground debris/bone prop at tile bounds `(18, 228)` through `(19, 229)` in `render_world/village_biome/houses`.
  - Add source `PropTile` metadata for missing source tile IDs 218, 219, 238, and 239.
  - Add object-level `PropMetadata` to object IDs 2290, 2291, 2296, and 2297 with `prop_instance_id=bone_debris_1_18_228`.
  - Preserve object positions, GIDs, layer membership, nearby trees/house objects, and runtime behavior.
  - Document that this low ground prop currently lives on the `houses` layer even though its semantic family is ground debris.
- Ticket:
  - done: Add source and placed-object metadata for the small village bone debris prop.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_house_fragment_slice119_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_house_fragment_slice119_nomarkers.png`.
  - Generated `.data/map-authoring-crops/world_village_house_blue2_context_slice119_objects.png`.
  - Generated `.data/map-authoring-crops/tileset_ground_debris_slice119_source.png`.
  - Visual inspection confirmed a low white ground debris/bone prop, not a partial house structure.
  - Added source `PropTile` metadata for tile IDs 218, 219, 238, and 239 as `bone_debris_1`.
- Evidence:
  - Source check confirmed tile IDs 218, 219, 238, and 239 use `class:"PropTile"`, `type:"bone_debris_1"`, `tile_kind=prop`, `occlusion_kind=none`, and low `render_height` values.
  - Source check confirmed object IDs 2290, 2291, 2296, and 2297 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=bone_debris_1`, `prop_kind=ground_debris`, and `prop_instance_id=bone_debris_1_18_228`.
...
### 2026-06-11 14:39 UTC - Manual Tiled Authoring Slice 118

- Status: done
- Scope:
  - Enrich the complete red-roof `house_red_1` exterior at tile bounds `(24, 222)` through `(31, 231)` in `render_world/village_biome/houses`.
  - Use visual inspection and existing typed `house_red_1` instances to infer exact object-level `PropMetadata`.
  - Reuse existing source `StructureTile` metadata for `house_red_1`, adding only source metadata gaps found during inspection.
  - Exclude overlapping tree, camp, path, and gameplay marker objects from this house slice.
  - Preserve object positions, GIDs, layer membership, nearby trees/camps/roaming areas, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the south village red-roof house exterior.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_red_house_slice118_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_red_house_slice118_nomarkers.png`.
  - Visual inspection confirmed a complete red-roof house exterior centered in the south village.
  - Gameplay inspection found a nearby rat roaming area but no aligned door object in the rendered slice.
  - Spark explorer independently mapped the target objects against existing typed `house_red_1` instances and confirmed no aligned gameplay door object.
  - Added missing source `StructureTile` metadata for tile IDs 210, 230, and 250 as non-occluding right-padding pieces.
  - Extended the initial 72-object scope to include the lower footprint row after the audit exposed GIDs 384 through 391 as the same exterior.
- Evidence:
  - Source check confirmed tile IDs 210, 230, and 250 are typed `StructureTile` records for `house_red_1` with non-occluding right-padding semantics.
  - Source check confirmed 80 objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_red_1`, `prop_kind=house_exterior`, and `prop_instance_id=house_red_1_24_222`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the adjacent two-column house/structure fragment around `(18..19, 228..229)`, then the next blue-house cluster around `(49..56, 231..238)`.

### 2026-06-11 14:33 UTC - Manual Tiled Authoring Slice 117

- Status: done
- Scope:
  - Enrich the complete 2x2 gray rock prop at tile bounds `(3, 222)` through `(4, 223)` in `render_world/village_biome/houses`.
  - Reuse existing `small_rock_2` source `PropTile` metadata for GIDs 257, 258, 277, and 278; do not edit `tilesheet.wang.tsj`.
  - Add object-level `PropMetadata` to object IDs 2252, 2253, 2266, and 2267 with `prop_instance_id=small_rock_2_3_222`.
  - Preserve object positions, GIDs, source collision objectgroups, nearby tree/large-rock objects, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the repeated west-edge village `small_rock_2` boulder.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice117_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice117_nomarkers.png`.
  - Visual inspection confirmed a standalone gray 2x2 boulder near the west village edge.
  - Source inspection confirmed GIDs 257, 258, 277, and 278 already carry `small_rock_2` `PropTile` metadata and collision objectgroups.
- Evidence:
  - Source check confirmed object IDs 2252, 2253, 2266, and 2267 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=small_rock_2`, `prop_kind=rock`, and `prop_instance_id=small_rock_2_3_222`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Inspect the red-house/structure cluster around `(24..31, 222..228)` before deciding whether it is one full exterior instance, multiple adjacent structures, or a partial edge assembly.

### 2026-06-11 14:27 UTC - Manual Tiled Authoring Slice 116

- Status: done
- Scope:
  - Enrich the complete 2x2 gray rock prop at tile bounds `(50, 218)` through `(51, 219)` in `render_world/village_biome/houses`.
  - Reuse existing `small_rock_2` source `PropTile` metadata for GIDs 257, 258, 277, and 278; do not edit `tilesheet.wang.tsj`.
  - Add object-level `PropMetadata` to object IDs 2208, 2209, 2221, and 2222 with `prop_instance_id=small_rock_2_50_218`.
  - Preserve object positions, GIDs, source collision objectgroups, nearby house/camp/tree/gameplay objects, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the repeated village `small_rock_2` boulder at `(50, 218)`.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice116_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice116_nomarkers.png`.
  - Visual inspection confirmed a standalone gray 2x2 boulder.
  - Source inspection confirmed GIDs 257, 258, 277, and 278 already carry `small_rock_2` `PropTile` metadata and collision objectgroups.
- Evidence:
  - Source check confirmed object IDs 2208, 2209, 2221, and 2222 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=small_rock_2`, `prop_kind=rock`, and `prop_instance_id=small_rock_2_50_218`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the next repeated `small_rock_2` instance around `(3..4, 222..223)` or the red-house edge/structure cluster around `(24..31, 222..228)`.

### 2026-06-11 14:23 UTC - Manual Tiled Authoring Slice 115

- Status: done
- Scope:
  - Enrich the complete `house_blue_2` exterior at tile bounds `(33, 217)` through `(40, 224)` in `render_world/village_biome/houses`.
  - Use `prop_instance_id=house_blue_2_33_217` because no aligned door object links this exterior to a specific house map.
  - Add object-level `PropMetadata` to all 64 connected roof, wall, side-padding, foundation, and lower footprint objects.
  - Reuse existing source `StructureTile` metadata for `house_blue_2`; do not edit `tilesheet.wang.tsj`.
  - Preserve object positions, GIDs, layer membership, nearby rock objects, non-houses render layers, roaming/checkpoint gameplay markers, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the south village `house_blue_2` exterior.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_house_blue2_slice115_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_house_blue2_slice115_nomarkers.png`.
  - Visual inspection confirmed a complete blue-roof house exterior.
  - Object inspection found nearby gameplay and non-houses render objects but no aligned house-entry door link.
  - Source inspection reused the already typed `house_blue_2` `StructureTile` records verified in Slice 114.
- Evidence:
  - Source check confirmed all 64 selected house objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_blue_2`, `prop_kind=house_exterior`, and `prop_instance_id=house_blue_2_33_217`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the adjacent `small_rock_2` instance around `(50..51, 218..219)` and the next red-house edge/structure cluster around `(24..31, 222..228)`.

### 2026-06-11 14:19 UTC - Manual Tiled Authoring Slice 114

- Status: done
- Scope:
  - Enrich the complete `house_blue_2` exterior at tile bounds `(11, 215)` through `(18, 222)` in `render_world/village_biome/houses`.
  - Use `prop_instance_id=house_blue_2_11_215` because no aligned door object links this exterior to a specific house map.
  - Add object-level `PropMetadata` to all 64 connected roof, wall, side-padding, foundation, and lower footprint objects.
  - Reuse existing source `StructureTile` metadata for `house_blue_2`; do not edit `tilesheet.wang.tsj`.
  - Preserve object positions, GIDs, layer membership, nearby well objects, static girl marker, checkpoint, deadlands objects, subterranean tree objects, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the southwest village `house_blue_2` exterior.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_structure_slice114_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_structure_slice114_nomarkers.png`.
  - Visual inspection confirmed a complete blue-roof house exterior with no aligned door object.
  - Source inspection confirmed selected GIDs resolve to typed `house_blue_2` `StructureTile` records.
- Evidence:
  - Source check confirmed all 64 selected house objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_blue_2`, `prop_kind=house_exterior`, and `prop_instance_id=house_blue_2_11_215`.
  - Terrain authoring audit reports 0 findings for the selected `house_blue_2_11_215` object IDs.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass after rerun; the first parallel invocation raced with `build:maps` and saw the map pack mid-refresh.
  - `bun run check:world-map:target`: pass.
  - `bun test tests/unit/terrain-authoring-audit.test.ts tests/unit/map-pack.test.ts tests/unit/world-map-validator.test.ts tests/unit/mmo/server-map-doors.test.ts --timeout 20000`: pass, 58 pass / 0 fail.
  - `bun run typecheck`: pass.
  - `bun run typecheck:tools`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the next repeated `house_blue_2` exterior around `(35..38, 217..219)` plus adjacent small-rock objects around `(50..51, 218..219)`.

### 2026-06-11 14:13 UTC - Manual Tiled Authoring Slice 113

- Status: done
- Scope:
  - Enrich the repeated complete 2x2 gray rock prop at tile bounds `(25, 214)` through `(26, 215)` in `render_world/village_biome/houses`.
  - Reuse existing `small_rock_2` source `PropTile` metadata from Slice 109; do not edit `tilesheet.wang.tsj`.
  - Add object-level `PropMetadata` to object IDs 2177, 2178, 2183, and 2184 with `prop_instance_id=small_rock_2_25_214`.
  - Preserve object positions, GIDs, source collision objectgroups, nearby house/tree objects, and runtime behavior.
- Ticket:
  - done: Add placed-object metadata for the repeated village `small_rock_2` boulder.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice113_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_gray_rock_slice113_nomarkers.png`.
  - Visual inspection confirmed a standalone low gray 2x2 boulder variant.
  - Source inspection confirmed GIDs 259, 260, 279, and 280 already carry `small_rock_2` `PropTile` metadata and collision objectgroups.
- Evidence:
  - Source check confirmed object IDs 2177, 2178, 2183, and 2184 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=small_rock_2`, `prop_kind=rock`, and `prop_instance_id=small_rock_2_25_214`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the next four-wide structure cluster around `(13..16, 215..218)`.

### 2026-06-11 14:09 UTC - Manual Tiled Authoring Slice 112

- Status: done
- Scope:
  - Enrich the complete `well_1` prop assembly at tile bounds `(6, 207)` through `(8, 211)` in `render_world/village_biome/houses`.
  - Use `prop_instance_id=well_1_6_207` because this is a freestanding village well with no aligned door/link.
  - Add object-level `PropMetadata` to the 15 connected well roof, right-padding, rim/base, bucket/interior, and cast-shadow objects.
  - Add source `StructureTile` metadata for the two anonymous right-padding source tiles ID 282 / GID 283 and ID 302 / GID 303.
  - Preserve object positions, GIDs, layer membership, existing collision objectgroups, nearby rock/house objects, and runtime behavior.
- Ticket:
  - done: Add placed-object and missing source metadata for the west village `well_1` instance.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_small_structure_slice112_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_small_structure_slice112_nomarkers.png`.
  - Visual inspection confirmed a freestanding small wooden-roof well near the west village cliff.
  - Compared against the earlier `well_1_61_201` instance and the `tileset_well_1_structure_tile_semantics.png` crop.
  - Added source metadata for right-padding tile IDs 282 and 302 as non-occluding `well_1` `StructureTile` records.
- Evidence:
  - Source check confirmed tile IDs 282 and 302 are typed `StructureTile` records for `well_1` with non-occluding right-padding semantics.
  - Source check confirmed all 15 selected well objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=well_1`, `prop_kind=well`, and `prop_instance_id=well_1_6_207`.
  - Terrain authoring audit reports 0 findings for the selected `well_1_6_207` object IDs and 0 `TILE_SEMANTIC*` findings.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  - `bun run check:world-map:target`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the repeated `small_rock_2` instance around `(25..26, 214..215)` or the next four-wide structure cluster around `(13..16, 215..218)`.

### 2026-06-11 14:04 UTC - Manual Tiled Authoring Slice 111

- Status: done
- Scope:
  - Enrich the eight lower footprint/padding objects directly below the existing `house_12_exterior` at tile bounds `(48, 207)` through `(55, 207)` in `render_world/village_biome/houses`.
  - Use the existing `prop_instance_id=house_12_exterior`, `prop_family=house_red_1`, and `prop_kind=house_exterior`.
  - Preserve object positions, GIDs, source tile metadata, layer membership, the `world_house_12_entry` door linkage, and runtime behavior.
  - Exclude neighboring trees, terrain, and all already-tagged house rows.
- Ticket:
  - done: Add the missing lower footprint row to the authored `house_12_exterior` object metadata.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_house_red1_slice111_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_house_red1_slice111_nomarkers.png`.
  - Visual inspection confirmed the row is the gray lower footprint/padding directly under the red-house doorway.
  - Object inspection confirmed the adjacent rows already use `prop_instance_id=house_12_exterior`.
- Evidence:
  - Source check confirmed object IDs 2120-2127 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_red_1`, `prop_instance_id=house_12_exterior`, and `asset_grid_row=9`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the small structure/prop cluster around `(6..8, 207..211)` or the repeated `small_rock_2` instance around `(25..26, 214..215)`.

### 2026-06-11 13:58 UTC - Manual Tiled Authoring Slice 110

- Status: done
- Scope:
  - Enrich the complete `house_red_1` exterior at tile bounds `(74, 199)` through `(81, 208)` in `render_world/village_biome/houses`.
  - Use `prop_instance_id=house_31_exterior` because the aligned door object is `world_house_31_entry` targeting `house_31`.
  - Add object-level `PropMetadata` to the selected exterior tile objects while preserving the door object, nearby tube, cliff guards, deadlands objects, positions, GIDs, layer membership, and runtime behavior.
  - Reuse existing source `StructureTile` metadata for `house_red_1` and `house_red_door_open`; do not expand the slice into unrelated world props.
- Ticket:
  - done: Add placed-object metadata for the linked `house_31` village red-house exterior.
- Key actions:
  - Generated `.data/map-authoring-crops/world_village_house_red1_slice110_objects.png`.
  - Generated `.data/map-authoring-crops/world_village_house_red1_slice110_nomarkers.png`.
  - Visually confirmed a complete red-house exterior with an open doorway at the northeast village edge.
  - Confirmed `world_house_31_entry` at `(77, 206)` targets `house_31`.
  - Confirmed the selected source tiles already carry typed `StructureTile` metadata.
- Evidence:
  - Source check confirmed 71 selected objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=house_red_1`, and `prop_instance_id=house_31_exterior`.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue with the leftover lower row from the `house_12_exterior` assembly, the next small rock instance, or the deadlands rock clusters surfaced by the audit.
...
- Evidence:
  - Source check confirmed tile IDs 258, 259, 278, and 279 are typed `PropTile` records for `small_rock_2` with collision objectgroups preserved.
  - Source check confirmed object IDs 2025, 2026, 2050, and 2051 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `prop_family=small_rock_2`, and `prop_instance_id=small_rock_2_5_203`.
  - Terrain authoring audit reports 0 findings for the selected rock object IDs.
  - Validation passed before commit `1d81908 continue authored prop metadata`: `bun run check:terrain-authoring`, `bun run build:maps`, `bun run check:maps`, `bun run check:world-map:target`, focused unit suite, `bun run typecheck`, `bun run typecheck:tools`, `bun run lint`, and `git diff --check`.
- Next action:
  - Continue with linked house exteriors and remaining rock/structure clusters surfaced by the audit.
...
- Status: done
- Scope:
  - Enrich four standalone GID 1826 beach shell/debris tile objects in `render_world/beach_biome/beach_props`.
  ...
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Tile ID 1825 carries complete typed `PropTile` metadata as a non-occluding `beach_shell` tile.
  - Object IDs 5121, 5133, 5140, and 5147 carry non-blank `PropMetadata` object metadata.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_gid1826_shell_variant_x10.png`.
  - Visual inspection identified GID 1826 as a fan-shaped beach shell/debris tile.
  - Added `PropTile` metadata to tile ID 1825 as `beach_shell/shell_fan`.
  - Added `PropMetadata` object metadata to object IDs 5121, 5133, 5140, and 5147.
- Evidence:
  - JSON parse check passed for `assets/maps/tiled/world.json` and `assets/maps/tiled/tilesheet.wang.tsj`.
  - Source check confirms tile ID 1825 is a `PropTile` record with `asset_family=beach_shell`, `asset_part=shell_fan`, `tile_kind=prop`, `occlusion_kind=none`, and `render_height=0`.
  - Source check confirms object IDs 5121, 5133, 5140, and 5147 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, and part-specific `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for object IDs 5121, 5133, 5140, and 5147.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Commit the current authored map-semantics batch.

### 2026-06-11 12:07 UTC - Manual Tiled Authoring Slice 94

- Status: done
- Scope:
  - Enrich the 12-object GID 736-740 beach fence assembly currently appearing as blank tile objects in `render_world/beach_biome/beach_props`.
  - Add missing source-tile semantics for tile IDs 735 through 739 as `beach_fence` structure pieces.
  - Preserve the existing collision objectgroups on tile IDs 736, 737, and 738.
  - Add object-level names and semantic properties tying the connected placed objects into one authored beach fence instance.
- Acceptance criteria:
  - Tile IDs 735 through 739 carry complete typed `StructureTile` metadata.
  - Existing source collision counts on tile IDs 736, 737, and 738 remain unchanged.
  - Object IDs 5105 through 5108, 5111 through 5114, and 5117 through 5120 carry non-blank `PropMetadata` object metadata.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_fence_cluster_context_x10.png`.
  - Visual inspection identified a wooden beach fence/rail assembly next to a large rock.
  - Source inspection found collision objectgroups on tile IDs 736, 737, and 738 only.
  - Added `StructureTile` metadata to tile IDs 735 through 739 as `beach_fence` pieces.
  - Added `PropMetadata` object metadata to object IDs 5105 through 5108, 5111 through 5114, and 5117 through 5120.
  - Grouped the 12 objects into `beach_fence_66_281`.
- Evidence:
  - Source check confirms tile IDs 735 through 739 are `StructureTile` records with `asset_family=beach_fence`, `tile_kind=structure`, `occlusion_kind=wall`, and `render_height=1`.
  - Source check confirms collision objectgroups remain on tile IDs 736, 737, and 738 only.
  - Source check confirms the selected fence objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=collision`, and stable `prop_instance_id=beach_fence_66_281`.
  - Terrain authoring audit reports 0 findings for the 12 selected fence object IDs.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting blank `beach_props` clusters into authored prop instances; remaining candidates include shell variants, additional palm pieces, tent pieces, and beach debris.

### 2026-06-11 12:02 UTC - Manual Tiled Authoring Slice 93

- Status: done
- Scope:
  - Enrich one six-tile `palm_tree_1` canopy assembly currently appearing as blank tile objects in `render_world/beach_biome/beach_props`.
  - Reuse existing source semantics for GIDs 1821, 1822, 1823, 1841, 1842, and 1843.
  - Add object-level names and semantic properties tying the six canopy tiles into one authored palm canopy prop instance.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Object IDs 1206 through 1211 carry non-blank `PropMetadata` object metadata.
  - The six object records share one stable prop instance id and keep part-specific names.
  - The canopy uses an explicit base-row depth so it sorts with the palm footprint rather than by top-row placement.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_palm_canopy_top_slice93_x10.png`.
  - Visual inspection identified a two-row palm canopy/crown assembly.
  - Checked runtime depth metadata handling in `shared/maps/processmap.ts`.
  - Added `PropMetadata` object metadata to object IDs 1206 through 1211.
  - Used `depth_mode=explicit`, `depth_row=263`, and `depth_offset=0` so the canopy sorts from the palm base row.
- Evidence:
  - Source check confirms the selected palm canopy objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=explicit`, `depth_row=263`, and stable `prop_instance_id=palm_canopy_64_258`.
  - Terrain authoring audit reports 0 findings for object IDs 1206 through 1211.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting blank `beach_props` clusters into authored prop instances; remaining candidates include shell variants and the larger GID 736-740 collision-bearing fence/rock cluster.

### 2026-06-11 11:56 UTC - Manual Tiled Authoring Slice 92

- Status: done
- Scope:
  - Enrich four standalone GID 1846 beach shell/debris tile objects in `render_world/beach_biome/beach_props`.
  - Add missing source-tile semantics for tile ID 1845 as a low decorative beach shell prop.
  - Add object-level names and semantic properties for each shell placement.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Tile ID 1845 carries complete typed `PropTile` metadata as a non-occluding `beach_shell` tile.
  - Object IDs 5080, 5099, 5104, and 5132 carry non-blank `PropMetadata` object metadata.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_gid1846_x10.png`.
  - Generated context crop `.data/map-authoring-crops/tileset_palm_frond_tip_context_x10.png`.
  - Visual inspection corrected the first guess: GID 1846 is a small beach shell/debris tile near the palm sheet area, not a palm frond.
  - Added `PropTile` metadata to tile ID 1845 as non-occluding `beach_shell` shell art.
  - Added `PropMetadata` object metadata to object IDs 5080, 5099, 5104, and 5132.
- Evidence:
  - Source check confirms tile ID 1845 is a `PropTile` record with `asset_family=beach_shell`, `asset_part=shell`, `tile_kind=prop`, `occlusion_kind=none`, and `render_height=0`.
  - Source check confirms the selected shell objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=bottom`, and stable placement-level `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for object IDs 5080, 5099, 5104, and 5132.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting blank `beach_props` clusters into authored prop instances; the larger GID 736-740 fence/rock cluster should be handled separately because it has collision-bearing source tiles and mixed missing metadata.

### 2026-06-11 11:51 UTC - Manual Tiled Authoring Slice 91

- Status: done
- Scope:
  - Enrich the remaining repeated two-tile `tree_1` root pairs currently appearing as blank tile objects in `render_world/beach_biome/beach_props`.
  - Reuse the already-authored source semantics for GIDs 312 and 313 from Slice 88.
  - Add object-level names and semantic properties tying each repeated pair into a distinct authored `tree_root` prop instance.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - The 24 selected tree-root object records carry non-blank `PropMetadata` object metadata.
  - The object records are grouped into 12 stable prop instance ids and keep left/right part names.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_tree_root_pair_slice91_x10.png`.
  - Visual inspection confirmed the same green/dark exposed `tree_1` root pair authored in Slice 88.
  - Located 12 remaining blank adjacent object pairs using GIDs 312 and 313.
  - Added `PropMetadata` object metadata to the 24 selected tree-root object records.
  - Grouped each pair into stable `tree_root_<tx>_<ty>` prop instance ids.
- Evidence:
  - Source check confirms the selected tree-root objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=bottom`, and stable pair-level `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for the 24 selected tree-root object IDs.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting blank `beach_props` clusters into authored prop instances; next candidates include single-tile beach rocks/debris and larger shoreline prop clusters.

### 2026-06-11 11:46 UTC - Manual Tiled Authoring Slice 90

- Status: done
- Scope:
  - Enrich 13 repeated two-tile `palm_tree_1` trunk-base pairs currently appearing as blank tile objects in `render_world/beach_biome/beach_props`.
  - Add missing source-tile semantics for tile ID 1900, the left half of the palm trunk base addressed by GID 1901.
  - Add object-level names and semantic properties tying each repeated pair into a distinct authored `palm_base` prop instance.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Tile ID 1900 carries complete typed `PropTile` metadata as a `palm_tree_1` trunk-base-left piece.
  - The 26 selected palm-base object records carry non-blank `PropMetadata` object metadata.
  - The object records are grouped into 13 stable prop instance ids and keep left/right part names.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_palm_base_pair_x10.png`.
  - Generated context crop `.data/map-authoring-crops/tileset_palm_base_context_x10.png`.
  - Visual inspection identified the pair as a palm trunk base with shadow, matching existing `palm_tree_1` trunk-base metadata on tile ID 1901.
  - Located 13 repeated adjacent object pairs using GIDs 1901 and 1902.
  - Added `PropTile` metadata to tile ID 1900 as `palm_tree_1` `trunk_base_left`.
  - Added `PropMetadata` object metadata to the 26 selected palm-base object records.
  - Grouped each pair into stable `palm_base_<tx>_<ty>` prop instance ids.
- Evidence:
  - Source check confirms tile ID 1900 is a `PropTile` record with `asset_family=palm_tree_1`, `asset_part=trunk_base_left`, `tile_kind=prop`, `occlusion_kind=trunk`, and `render_height=0`.
  - Source check confirms the selected palm-base objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=collision`, and stable pair-level `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for the 26 selected palm-base object IDs.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting repeated blank `beach_props` clusters into authored prop instances; likely next candidates include remaining tree roots and single-tile beach debris/rocks.

### 2026-06-11 11:41 UTC - Manual Tiled Authoring Slice 89

- Status: done
- Scope:
  - Enrich two repeated `tent_1` front assemblies currently appearing as blank tile objects in `render_world/beach_biome/beach_props`.
  - Add missing source-tile semantics for tile ID 202, the far-right bottom tent/shadow tile addressed by GID 203.
  - Add object-level names and semantic properties tying each seven-object tent assembly into a distinct authored `tent_1` prop/structure instance.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Tile ID 202 carries complete typed `StructureTile` metadata as a `tent_1` base/shadow piece.
  - Object IDs 5062 through 5074 and 5077 through 5079 carry non-blank `PropMetadata` object metadata.
  - The fourteen object records are grouped into two stable prop instance ids and keep part-specific names.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_tent_placed_fragment_x10.png`.
  - Generated context crop `.data/map-authoring-crops/tileset_tent_context_x10.png`.
  - Visual inspection identified two repeated `tent_1` front assemblies with canvas wall, doorway flap/header, base flap, and dark right-side base/shadow tile.
  - Added `StructureTile` metadata to tile ID 202 as `tent_1` `base_shadow_right`.
  - Added `PropMetadata` object metadata to object IDs 5062 through 5074 and 5077 through 5079.
  - Grouped the fourteen objects into `tent_front_16_261` and `tent_front_70_265`.
- Evidence:
  - Source check confirms tile ID 202 is a `StructureTile` record with `asset_family=tent_1`, `asset_part=base_shadow_right`, `tile_kind=structure`, `occlusion_kind=none`, and `render_height=0`.
  - Source check confirms the selected tent objects use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=collision`, and stable instance-level `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for object IDs 5062 through 5074 and 5077 through 5079.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting repeated blank `beach_props` clusters into authored prop instances; likely next candidates include additional tree roots, small rocks/debris, and shoreline prop pairs.

### 2026-06-11 11:34 UTC - Manual Tiled Authoring Slice 88

- Status: done
- Scope:
  - Enrich the repeated two-tile `tree_1` root pairs currently appearing as blank tile objects in `render_world/beach_biome/beach_props`.
  - Add missing source-tile semantics for the two root tiles addressed by GIDs 312 and 313.
  - Add object-level names and semantic properties tying each repeated pair into a distinct authored `tree_root` prop instance.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Tile IDs 311 and 312 carry complete typed `PropTile` metadata as `tree_1` root pieces.
  - Object IDs 5056 through 5061 carry non-blank `PropMetadata` object metadata.
  - The six object records are grouped into three stable prop instance ids and keep left/right part names.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_gid312_313_x10.png`.
  - Generated wider root-row crop `.data/map-authoring-crops/tileset_tree_root_row_gap_x10.png`.
  - Visual inspection identified the pair as a green/dark exposed tree-root base, aligned with the existing `tree_1` root row.
  - Added `PropTile` metadata to tile IDs 311 and 312 as `tree_1` root pieces.
  - Added `PropMetadata` object metadata to object IDs 5056 through 5061.
  - Grouped the six objects into `tree_root_69_255`, `tree_root_46_257`, and `tree_root_58_258`.
- Evidence:
  - Source check confirms tile IDs 311 and 312 are `PropTile` records with `asset_family=tree_1`, `asset_part=root_left/root_right_shadow`, `tile_kind=prop`, `occlusion_kind=trunk`, and `render_height=0`.
  - Source check confirms object IDs 5056 through 5061 use `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, `depth_mode=bottom`, and stable pair-level `prop_instance_id` values.
  - Terrain authoring audit reports 0 findings for object IDs 5056 through 5061.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting repeated blank `beach_props` clusters into authored prop instances, while watching for more placed GIDs that expose missing source-tile metadata.

### 2026-06-11 11:28 UTC - Manual Tiled Authoring Slice 87

- Status: done
- Scope:
  - Enrich one visible `beach_umbrella` renderable tile-object instance in `render_world/beach_biome/beach_props`.
  - Reuse the existing `PropMetadata` template convention for renderable prop objects.
  - Add object-level names and semantic properties tying the four 2x2 umbrella tile objects into one authored prop instance.
  - Preserve object positions, GIDs, layer membership, and runtime map generation behavior.
- Acceptance criteria:
  - Object IDs 1403, 1404, 5134, and 5135 carry non-blank object metadata.
  - The four object records share one stable prop instance id and keep variant-specific part names.
  - The selected object IDs disappear from `tile_object` findings without adding new semantic-tile findings.
  - The capped audit summary behavior is documented if the aggregate `tile_object` count remains unchanged.
  - Map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Located the four `beach_umbrella` tile objects at grid anchor 77,293 using GIDs 481, 482, 501, and 502.
  - Compared against existing `PropMetadata` object usage in badlands cactus and totem objects.
  - Added names, `template:"templates/prop_metadata.tx"`, `type:"PropMetadata"`, shared `prop_instance_id=beach_umbrella_77_293`, and per-part metadata to object IDs 1403, 1404, 5134, and 5135.
- Evidence:
  - Source check confirms object IDs 1403, 1404, 5134, and 5135 are on `render_world/beach_biome/beach_props`, share `prop_instance_id=beach_umbrella_77_293`, and keep part-specific `asset_part` values.
  - Terrain authoring audit reports 0 findings for object IDs 1403, 1404, 5134, and 5135.
  - Aggregate `tile_object` finding count remains 350 because the audit caps blank-object reports per object layer and `beach_props` still has more blank objects beyond the selected instance.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue converting blank renderable tile objects into authored prop instances, starting with repeated beach prop clusters or existing badlands totem/cactus groups.

### 2026-06-11 11:20 UTC - Manual Tiled Authoring Slice 86

- Status: done
- Scope:
  - Enrich the four remaining unclassified totem body source families: `totem_1`, `totem_2`, `totem_3`, and `totem_4`.
...
- Acceptance criteria:
  - All 16 totem body records carry complete typed `PropTile` metadata.
  - Existing per-family collision and footprint counts remain 1 and 1.
...
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_totem_bodies_before_semantics.png`.
  - Generated context crop `.data/map-authoring-crops/tileset_totem_bodies_context_x10.png`.
  - Visual inspection identified four adjacent carved log/totem body variants with cap/head, upper body, lower body, and base rows.
  - Tagged 16 `totem_1` through `totem_4` tile records as `PropTile`.
  - Classified all body rows as trunk/body occlusion with render heights 3, 2, 1, and 0.
  - Preserved the existing base collision and footprint tile for each totem variant.
  - Generated `.data/map-authoring-crops/tileset_totem_bodies_prop_tile_semantics.png`.
  - Updated `docs/terrain-authoring-model.md` and `docs/tiled-authoring-rethink.md`.
- Evidence:
  - Source audit confirms `PropTile:totem_1=4`, `PropTile:totem_2=4`, `PropTile:totem_3=4`, and `PropTile:totem_4=4`.
  - Source audit confirms each totem body family preserved 1 footprint tile and 1 collision tile, with 0 semantic metadata leaks into collision object properties.
  - Source-family semantic class coverage reports no remaining typed legacy source families without `PropTile`, `StructureTile`, or `TransitionTile`.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Shift from source-family classification to richer map/layer authoring semantics, transition assets, and remaining audit debt categories.

### 2026-06-11 11:16 UTC - Manual Tiled Authoring Slice 85
...
- Acceptance criteria:
  - All 4 `beach_umbrella` records carry complete typed `PropTile` metadata.
  - Existing `beach_umbrella` collision and footprint counts remain 2 and 2.
  - No semantic tile metadata is written into nested collision object properties.
  - Terrain authoring, map, typecheck, lint, and focused unit gates pass.
- Key actions:
  - Generated `.data/map-authoring-crops/tileset_beach_umbrella_before_semantics.png`.
  - Generated context crop `.data/map-authoring-crops/tileset_beach_umbrella_context_x10.png`.
  - Visual inspection identified a 2x2 blue-white beach umbrella with upper canopy tiles and a lower pole/shadow footprint row.
  - Tagged 4 `beach_umbrella` tile records as `PropTile`.
  - Classified the upper row as canopy occlusion and the lower row as trunk/body occlusion while preserving collision and footprint.
  - Generated `.data/map-authoring-crops/tileset_beach_umbrella_prop_tile_semantics.png`.
  - Updated `docs/terrain-authoring-model.md` and `docs/tiled-authoring-rethink.md`.
- Evidence:
  - Source audit confirms `PropTile:beach_umbrella=4`.
  - Source audit confirms `beach_umbrella` preserved 2 footprint tiles and 2 collision tiles, with 0 semantic metadata leaks into collision object properties.
  - Generated terrain audit reports 0 `TILE_SEMANTIC_METADATA_MISSING`, 0 `TILE_SEMANTIC_ASSET_FAMILY_MISMATCH`, 0 `TILE_SEMANTIC_ENUM_UNTYPED`, 0 `TILE_SEMANTIC_KIND_MISMATCH`, and 0 `TILE_SEMANTIC_RENDER_HEIGHT_INVALID`.
  - `bun run check:terrain-authoring`: pass.
  - `bun run build:maps`: pass.
  - `bun run check:maps`: pass.
  ...
  - `git diff --check`: pass.
- Next action:
  - Continue source-asset enrichment on the four totem body variants.
