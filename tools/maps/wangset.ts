...
const MAP_RULE_MATCHER = "world*";

const scriptDir = path.dirname(fileURLToPath(import.meta.url));
const defaultSourcePath = path.join(scriptDir, "../../assets/maps/tiled/world.json");
const defaultTilesetOutputPath = path.join(scriptDir, "../../assets/maps/tiled/tilesheet.wang.tsj");
const defaultRulesPath = path.join(scriptDir, "../../assets/maps/tiled/automapping.rules");
...
async function listRuleMaps(rulesPath: string): Promise<string[]> {
    const rulesDir = path.join(path.dirname(rulesPath), "rules");
    const entries = await fs.readdir(rulesDir, { withFileTypes: true }).catch(() => []);

    return entries
        .filter((entry) => entry.isFile() && entry.name.toLowerCase().endsWith(".tmj"))
        .map((entry) => toPosixPath(path.join("rules", entry.name)))
        .sort((a, b) => a.localeCompare(b));
}

function buildAutomappingRulesContent(ruleMaps: string[]): string {
    const lines = [
        "# BrowserQuest Tiled Automapping rules",
        "# Generated by tools/maps/wangset.ts",
        "# Add or update rule maps in assets/maps/tiled/rules/*.tmj",
        "",
        `[${MAP_RULE_MATCHER}]`,
        ...ruleMaps,
    ];
...
export async function syncWangsetArtifacts(options?: {
    ...
    const ruleMaps = await listRuleMaps(rulesPath);
    if (ruleMaps.length === 0) {
        throw new Error(
            "map-wang-sync: no automapping rule maps found in assets/maps/tiled/rules/*.tmj. Add at least one rule map."
        );
    }

    const tilesetContent = stableStringify(artifacts.tilesetDocument);
    const rulesContent = buildAutomappingRulesContent(ruleMaps);
        ...
        if (drifted) {
            ...
            throw new Error(
                `map-wang-sync-check: artifacts out of sync:\n - ${driftedFiles.join(
                    "\n - "
                )}\nRun \`bun run map:wang:sync\` to regenerate terrain wang and automapping artifacts.`
            );
        }
        if (!quiet) {
            console.log(
                `map-wang-sync-check: ok (${artifacts.wangTileCount} wang tiles across ${artifacts.terrainCount} terrain colors, ${ruleMaps.length} rule maps).`
            );
        }
        return {
            changed: false,
    ...
    if (!quiet) {
        console.log(
            `map:wang:sync ${tilesetChanged || rulesChanged ? "updated" : "up-to-date"} (${artifacts.wangTileCount} wang tiles, ${artifacts.terrainCount} terrain colors, ${ruleMaps.length} rule maps).`
        );
    }
