...
function buildAutomappingRulesContent(ruleMaps: string[]): string {
    const lines = [
        ...
        "# Add or update rule maps in tools/maps/tiled/rules/*.tmj",
        "",
        `[${MAP_RULE_MATCHER}]`,
        ...ruleMaps,
    ];
...
export async function syncWangsetArtifacts(options?: {
    ...
    if (ruleMaps.length === 0) {
        throw new Error(
            "map-wang-sync: no automapping rule maps found in tools/maps/tiled/rules/*.tmj. Add at least one rule map."
        );
    }
