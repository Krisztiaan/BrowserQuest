...
if (viteConfigSource.includes('browserquest-map-runtime-sync') || viteConfigSource.includes('syncRuntimeMaps(')) {
...
if (legacyRootDocs.length > 0) {
    fail(`legacy historical docs must live under docs/archive/legacy (found: ${legacyRootDocs.join(', ')})`);
}
...
if (archivedNoteOffenders.length > 0) {
    fail(`archived-note docs must be moved under docs/archive (found: ${archivedNoteOffenders.join(', ')})`);
}
