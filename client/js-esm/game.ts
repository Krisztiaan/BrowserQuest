...
class Game {
    ...
    constructor(app) {
        ...
        // combat
        this.infoManager = new InfoManager(this as any);
        ...
        // sprites
        this.spriteNames = [...SPRITE_KEYS] as SpriteKey[];
    }
    ...
    setCursor(name: CursorKey, orientation?) {
    ...
    connect(started_callback) {
        ...
        this.client.onEntityList(function(list) {
            var entityIds = Object.values(self.entities).map(function(entity: any) { return entity.id; }),
                knownIds = entityIds.filter(function(id) { return list.includes(id); }),
                newIds = list.filter(function(id) { return !knownIds.includes(id); });
        
            self.obsoleteEntities = Object.values(self.entities).filter(function(entity: any) {
                            ...
                            if(entity instanceof Character) {
                                ...
                                entity.onDeath(function() {
                                    ...
                                    if(self.camera.isVisible(entity)) {
                                        const killSound = (
                                            Math.floor(Math.random() * 2 + 1) === 1 ? 'kill1' : 'kill2'
                                        ) as AudioSoundKey;
                                        self.audioManager.playSound(killSound);
                                    }
            ...
            if(character.canAttack(time)) {
                if(!isMoving) { // don't hit target if moving to a different tile.
                    ...
                    if(character instanceof Player && this.camera.isVisible(character)) {
                        const hitSound = (
                            Math.floor(Math.random() * 2 + 1) === 1 ? 'hit1' : 'hit2'
                        ) as AudioSoundKey;
    ...
    removeObsoleteEntities() {
        ...
        if(nb > 0) {
            ...
            log.debug("Removed "+nb+" entities: "+obsoleteEntities.filter(function(entity) {
